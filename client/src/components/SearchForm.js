    ...
    <div className="grid">
        <div>
            ...
            <TextField onChange={ (e) => {
                handleValidation(e); refetch({"name": e.target.value}).then(data => setStateData(data.data.number)); }} 
            error={!valid} helperText={valid ? "" : "Please use letters only"}/>
        </div>
       <div className="cardContainer">
            {stateData?.map(element => <NameCard element={element}/>)}
        </div>
    </div>
    )
}